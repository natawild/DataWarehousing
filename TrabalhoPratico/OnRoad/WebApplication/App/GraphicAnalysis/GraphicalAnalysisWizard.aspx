...
<script language="javascript">
	...
	function BAonload(){
			...
			for (var i = 0; i < allMeasures.length; i++){
				listContainer.getElementsByTagName("tbody")[0].appendChild( renderWizardMeasures(allMeasures[i].columnName) );
			}
			
			var dropContent = document.getElementById("dropContent");
			dropContent.innerHTML = "";
			for (var i = 0; i < allDimensions.length; i++){
				var dd = renderWizardDimension(allDimensions[i].columnName);
...
<script>
	...
	function initDrag(){
		...
		for (var i = 0; i < allDimensions.length; i++){
			dragDropObj.addSource('box_' + allDimensions[i].columnName, true, true, true, false,'onDragFunction');
		}	
