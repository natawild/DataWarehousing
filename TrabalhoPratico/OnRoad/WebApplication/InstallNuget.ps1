...
function Copy-JQueryContentToProject
{
    ...
    $nugetDirectory = [io.path]::combine((Get-SolutionPackagesDirectory -ProjectDir $projectDir),"$nugetName.$nugetVersion")
    if( -not (Test-Path $nugetDirectory) )
    {
        $nugetDirectory = [io.path]::Combine((Get-UserNugetPackagesDirectory -NugetName $nugetName), $nugetVersion)
    }
