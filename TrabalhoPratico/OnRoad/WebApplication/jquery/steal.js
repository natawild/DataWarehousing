...
(function (win, undefined) {
    ...
    extend(URI.prototype, {
        ...
        join: function (uri, min) {
            ...
            if (uri.isRelativeToDomain()) {
                var additionalUri = uri && uri.path ? uri.path : uri;
                return URI(this.domain() + additionalUri);
            }
    ...
    */
    steal.idToUri = function (id, noJoin) {
        //SharePoint define variables diferent
        if (id.protocol)
            return id;
        // this is normalize
...
extend(Resource.prototype, {
    ...
    executed: function (script) {
        ...
        if (support.interactive && src && this.options.type != "fn") {
    ...
    types: {
        ...
        "text": function (options, success, error) {
            steal.request(options, function (text) {
                options.text = text;
                // Add to loaded text files
                steal.loadedTextFiles[options.id.path] = text;
                success(text);
            }, function () {
                error(arguments);
                if (options.onError) {
        ...
        "css": function (options, success, error) {
            if (options.text) { // less
                ...
                if (createSheet) {
                    ...
                    if (!cssCount++) {
                        lastSheet = doc.createStyleSheet();
                        lastSheet.addImport(addSuffix(options.src));
...
var factory = function () {
    // DEPL: Also  detect windows 8 flag
    return win.ActiveXObject && typeof Windows === "undefined" ? new ActiveXObject("Microsoft.XMLHTTP") : new XMLHttpRequest();
};
...
// CUSTOM TEXT LOADER EXTENSION
steal.loadedTextFiles = {};