		...
		for(var i = 0; i < inputs.length; i++){
			if (inputs[i].id.indexOf("check__") == 0){
				...
				if (check.checked){
					...
					if (!measure){
						measure = new Measure(sColumnName, "String");
	...
	function addWizardDimensions(){
		...
		for(var i = 0; i < dl.childNodes.length; i++){
			...
			if (dd && dd.id && dd.id.indexOf("dragableElement") == -1){
				var sColumnName = dd.id.replace("box_", "");
				var dimension = findItemByColumnName(sColumnName, allDimensions);
...
*/				
	...
	function renderWizardMeasures(sColumnName, sFunctions){
		...
		var tnText = document.createTextNode(getFriendlyName(sColumnName.replace("\"", "\\\"")));
		...
		if (sColumnName != "Counter"){
			divTextTableBody.appendChild( createTR(sColumnName, getFriendlyName("FunctionMax"), "max", "MaxIcon.gif", sFunctions.indexOf('max') > -1 ) );
			divTextTableBody.appendChild( createTR(sColumnName, getFriendlyName("FunctionMin"), "min", "MinIcon.gif", sFunctions.indexOf('min') > -1 ) );
			divTextTableBody.appendChild( createTR(sColumnName, getFriendlyName("FunctionAvg"), "avg", "AverageIcon.gif", sFunctions.indexOf('avg') > -1 ) );
			divTextTableBody.appendChild( createTR(sColumnName, getFriendlyName("FunctionSum"), "sum", "SumIcon.gif", sFunctions.indexOf('sum') > -1 ) );
		}
		else
			divTextTableBody.appendChild( createTR(sColumnName, getFriendlyName("FunctionCount"), "count", "CountIcon.gif", sFunctions.indexOf('count') > -1 ) );
	...
	function createTR(sColumnName, sName, sFunction, sImage, bChecked){
		...
		divTextTableTd1Check.id = "check__" + sColumnName + "__" + sFunction;
	...
	function renderWizardDimension(sColumnName){
		var dd = document.createElement("dd");
		dd.id = "box_" + sColumnName;
