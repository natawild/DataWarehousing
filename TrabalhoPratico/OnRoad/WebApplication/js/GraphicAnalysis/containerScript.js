...
var hasChanges = false;
	...
	function Measure(columnName, dataType){
		this.columnName = columnName;
		this.dataType = dataType;
	...
	function renderDimension(){
		...
		aX.href = "javascript:removeDimension('" + this.columnName + "', true);";
		...
		aNomMedida.onmouseover = "showhideinner('" + "innerTable_" + this.columnName + "',true)";
		aNomMedida.setAttribute("onmouseover", "showhideinner('" + "innerTable_" + this.columnName + "',true)");
		
		aNomMedida.onmouseout = "showhideinner('" + "innerTable_" + this.columnName + "',false)";
		aNomMedida.setAttribute("onmouseout", "showhideinner('" + "innerTable_" + this.columnName + "',false);");
		...
		table.id = "innerTable_" + this.columnName;
...
//		alert(li.outerHTML);
	...
	function createDimensionListItem(dimension, sAction){
		...
		if (sAction == "filter"){
			a.href = "javascript:showFilter('" + dimension.columnName + "');";
			img.src = "../../img/graphicanalysis/filterIcon.gif";
		}
		else if (sAction == "down"){
			a.href = "javascript:moveDown('" + dimension.columnName + "');";
			img.src = "../../img/graphicanalysis/downitemmedida.gif";
		}
		else if (sAction == "up"){
			a.href = "javascript:moveUp('" + dimension.columnName + "');";
	...
	function showDimensionDateFilter() //dimension, startDate, endDate
	{
		...
		ShowBAWindowModal(getFriendlyName( this.columnName), 350, 250, "ShowDateRanges.aspx?h_dimension=" + this.columnName + "&start=" + this.startDate + "&end=" + this.endDate + "&selected=" + selectedValue);
	...
	{
		var selectedValue = this.filter;
		ShowBAWindowModal( getFriendlyName( this.columnName ), 450, 400, 'DimensionFilter.aspx?h_Dimension=' + this.columnName);
	}
	...
	function renderMeasureContainer(selectedMeasures){
		...
		if (selectedMeasures.length == 0){
			selectedMeasures[0] = new Measure("Counter", "Int32");
	...
	function findDimensionByColumnName(sColumnName, dimensionsArray){  //selectedDimensions
		return findItemByColumnName(sColumnName, dimensionsArray);
	}
